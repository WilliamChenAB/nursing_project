...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public class CaptureBase : MonoBehaviour
	{
		...
		private int _selectedAudioInputDeviceIndex = 0;
		...
		public int ForceAudioInputDeviceIndex
		{
			...
			set { _forceAudioInputDeviceIndex = value; SelectAudioDevice(); }
		}
		...
		public int SelectedAudioInputDeviceIndex
		{
			get { return _selectedAudioInputDeviceIndex; }
		}

		public string SelectedAudioInputDeviceName
		{
			get
			{
				if (_selectedAudioInputDeviceIndex >= 0 && _selectedAudioInputDeviceIndex < NativePlugin.GetAudioInputDeviceCount())
					return NativePlugin.GetAudioInputDeviceName(_selectedAudioInputDeviceIndex);
				return "None";
			}
		}
		...
		private static bool SelectCodec(ref Codec codec, CodecList codecList, int forceCodecIndex, string[] codecPriorityList, MediaApi matchMediaApi, bool allowFallbackToFirstCodec, bool logFallbackWarning)
		{
			...
			{
				if (forceCodecIndex < CodecManager.VideoCodecs.Count)
				{
			...
			if (codec == null && allowFallbackToFirstCodec)
			{
				...
				{
					codec = CodecManager.VideoCodecs.GetFirstCodecByMediaApi(matchMediaApi);
					if (logFallbackWarning)
					{
						Debug.LogWarning("[AVProMovieCapture] Video codec not found.  Video will be use the first codec available.");
					}
				}
			}
		...
		{
			_selectedAudioInputDeviceIndex = -1;
			if (_forceAudioInputDeviceIndex >= 0 && _forceAudioInputDeviceIndex < NativePlugin.GetAudioInputDeviceCount())
			{
				_selectedAudioInputDeviceIndex = _forceAudioInputDeviceIndex;
			}
		}
		...
		public virtual bool PrepareCapture()
		{
			...
			if (_audioCaptureSource != AudioCaptureSource.None && _outputTarget == OutputTarget.VideoFile && _isRealTime)
			{
				...
				{
					if (_selectedAudioInputDeviceIndex >= 0)
					{
						audioDeviceIndex = _selectedAudioInputDeviceIndex;
			...
			if (_logCaptureStartStop)
			{
				...
				if (_outputTarget == OutputTarget.VideoFile)
				{
					...
					{
						if (_audioCaptureSource == AudioCaptureSource.Microphone && _selectedAudioInputDeviceIndex >= 0)
						{
							info += string.Format(" audio source:'{0}'", SelectedAudioInputDeviceName);
						}
						else if (_audioCaptureSource == AudioCaptureSource.Unity)
						{
			...
			if (_outputTarget == OutputTarget.VideoFile)
			{
				...
				_handle = NativePlugin.CreateRecorderVideo(_filePath, (uint)_targetWidth, (uint)_targetHeight, _frameRate, (int)_pixelFormat, useRealtimeClock, _isTopDown,
																	...
																	_stats.UnityAudioChannelCount, audioDeviceIndex, (_selectedAudioCodec != null)?_selectedAudioCodec.Index:-1,
																	_forceGpuFlush, GetEncoderHints().videoHints);
			}
			else if (_outputTarget == OutputTarget.ImageSequence)
			{
				if (_logCaptureStartStop)
