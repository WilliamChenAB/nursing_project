...
namespace RenderHeads.Media.AVProMovieCapture.Editor
{
	...
	public class CaptureEditorWindow : EditorWindow
	{
		...
		private const string LinkAssetStorePage = "https://assetstore.unity.com/packages/slug/151061";
		...
		private void LoadSettings()
		{
			...
			_audioDeviceIndex = EditorPrefs.GetInt(SettingsPrefix + "AudioDeviceIndex", _audioDeviceIndex);
		...
		private void Configure(CaptureBase capture)
		{
			...
			if (capture.AudioCaptureSource != AudioCaptureSource.None)
			{
				...
				{
					capture.ForceAudioInputDeviceIndex = _audioDeviceIndex;
				}
			}
		...
		void OnInspectorUpdate()
		{
			if (_capture != null)
			{
				if (Application.isPlaying)
				{
					...
					if (_capture.StopMode != StopMode.None && _capture.CaptureStats.NumEncodedFrames > 0 && !_capture.IsCapturing() && !_capture.IsStartCaptureQueued())
					{
						StopCapture();
					}
				}
		...
		private struct CodecMenuData
		{
			public CodecMenuData(Codec codec)
			{
				this.codec = codec;
			}
		...
		private void CodecMenuCallback_Select(object obj)
		{
			CodecMenuData data = (CodecMenuData)obj;

			if (data.codec.CodecType == CodecType.Video)
			{
				_videoCodec = data.codec;
			}
			else if (data.codec.CodecType == CodecType.Audio)
			{
				_audioCodec = data.codec;
			}

			UpdateSelectedCodec();
		}

		private GenericMenu CreateCodecMenu(CodecList codecList, Codec selectedCodec, Codec matchCodecMediaType)
		{
			...
			foreach (Codec c in codecList.Codecs)
			{
				bool isEnabled = (matchCodecMediaType == null || matchCodecMediaType.MediaApi == c.MediaApi);
#if UNITY_EDITOR_WIN
				if (c.MediaApi != lastApi)
				{
					string title = string.Empty;
					switch (c.MediaApi)
					{
						case MediaApi.DirectShow:
						title = "DirectShow Legacy Codecs:";
						break;
						case MediaApi.MediaFoundation:
						title = "Media Foundation MP4 Codecs:";
						break;
					}
					menu.AddSeparator("");
				...
				if (isEnabled)
				{
					menu.AddItem(new GUIContent(c.Name), c == selectedCodec, CodecMenuCallback_Select, new CodecMenuData(c));
				}
				else
				{
					menu.AddDisabledItem(new GUIContent(c.Name));
				}
			}
		...
		private bool ShowCodecList(string title, CodecList codecList, ref Codec selectedCodec, Codec matchCodecMediaType = null)
		{
			bool result = false;

			if (codecList == null || selectedCodec == null)
			{
				return result;
			}
			...
			if (EditorGUI.DropdownButton(rect, new GUIContent(selectedCodec.Name), FocusType.Keyboard))
			{
				CreateCodecMenu(codecList, selectedCodec, matchCodecMediaType).DropDown(rect);
			}

#if UNITY_EDITOR_WIN
			{
				EditorGUI.BeginDisabledGroup(!selectedCodec.HasConfigwindow);
		...
		public static void DrawBaseCapturingGUI(CaptureBase capture)
		{
			...
			if (capture.AudioCaptureSource != AudioCaptureSource.None)
			{
				...
				{
					EditorGUILayout.LabelField("Source", capture.SelectedAudioInputDeviceName);
				}
		...
		private void UpdateSelectedCodec()
		{
			...
			if (_videoCodec != null)
			{
				...
				if (_audioCodec != null)
				{
					if (_audioCodec.MediaApi != _videoCodec.MediaApi)
					{
						_audioCodec = CodecManager.AudioCodecs.GetFirstCodecByMediaApi(_videoCodec.MediaApi);
					}
				}
			}
		...
		private void DrawVisualCodecList()
		{
			...
			if (_outputTarget == OutputTarget.VideoFile)
			{
				...
				if (ShowCodecList("Video Codec", CodecManager.VideoCodecs, ref _videoCodec, null))
				{
		...
		{
			return (_audioCaptureSource != AudioCaptureSource.None && _outputTarget == OutputTarget.VideoFile && IsCaptureRealTime() && !IsUsingMediaFoundationAndMicrophone());					
		}

		private bool IsUsingMediaFoundationAndMicrophone()
		{
			return (_audioCaptureSource == AudioCaptureSource.Microphone && _videoCodec != null && _videoCodec.MediaApi == MediaApi.MediaFoundation);
		}
		...
		private void DrawAudioCodecList()
		{
			...
			if (ShowCodecList("Audio Codec", CodecManager.AudioCodecs, ref _audioCodec, _videoCodec))
			{
		...
		private void DrawConfigGUI_Audio()
		{
			...
			if (showAudioOptions)
			{
				...
				{
					if (IsUsingMediaFoundationAndMicrophone())
					{
						GUILayout.Space(8f);
						GUI.color = Color.yellow;
						GUILayout.TextArea("Audio capture from Microphone is not currently supported when using a Media Foundation codec");
						GUI.color = Color.white;
					}
					GUILayout.Space(8f);

					EditorGUI.BeginDisabledGroup(IsUsingMediaFoundationAndMicrophone());
					bool itemChanged = false;
					Codec dummyCodec = null;
					if (ShowConfigList("Microphone", _audioDeviceNames, null, ref dummyCodec, ref _audioDeviceIndex, ref itemChanged, false))
					{
					}
